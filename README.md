...
Another takeaway: having a plan (even a loose one) for a project as big as this really helps. Adding new goals on the fly felt exciting, but it also made the project drag on way longer than it needed to - sure, life got a bit in the way of this project, but still, this took me a long while to actually complete. While this was a fun project, at this point of my studying journey it's more important to actually complete the course instead of getting "stuck" like this.
...
3) I thought about adding an option for player to reply (similar to how occasionally player in Undertale can select one of the 2 replies in certain battle sequences), but decided against that - as that would only additionally complicate the "rate" logic, and replying mechanic wouldn't be used anywhere else in this game - all interaction is done by player ACTing instead.
...
Roadmap to project finalization - all done!!!
...
9) <s>Work on scaling for different/smaller screen resolutions - another intensive task</s>
...
10) "Mercy" doesn't apply properly if Mettaton's screen resolution as changed, still leaves the drawing on-screen - resolve the issue